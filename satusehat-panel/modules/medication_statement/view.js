/* ============================================================
   Module View: MedicationStatement (Pernyataan Penggunaan Obat)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Medication Statement...</div>`;
    ...
    try {
        const res = await api(`/api/modules/medication_statement/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada pernyataan penggunaan obat yang sesuai filter.</div>`;
        return;
    }
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.kode_brng}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/medication_statement/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `MedicationStatement (${r.nama_brng || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.kode_brng || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Statement ${itemKey.kode_brng}`, 'info');
                const res = await api('/api/modules/medication_statement/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `MedicationStatement terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
