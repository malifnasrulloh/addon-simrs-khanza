/* ============================================================
   Module View: CarePlan (Rencana Asuhan Medis / RTL)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Care Plan...</div>`;
    ...
    try {
        const res = await api(`/api/modules/care_plan/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada rencana tindak lanjut (RTL) yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span>${escapeHtml((r.rtl || '-').slice(0, 120))}</span>
                                <span class="blocker-tip">${escapeHtml(r.status_rawat || 'Ralan')} · ${escapeHtml(r.kd_poli || '')}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/care_plan/preview/${encodeURIComponent(keyStr)}`);
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim CarePlan`, 'info');
                const res = await api('/api/modules/care_plan/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `CarePlan terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
