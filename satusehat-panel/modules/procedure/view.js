/* ============================================================
   Module View: Procedure (Tindakan & Prosedur Medis)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Procedure...</div>`;
    ...
    try {
        const res = await api(`/api/modules/procedure/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada tindakan/prosedur yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.kode}|${r.status_prosedur}`;
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong class="mono">${escapeHtml(r.kode)}</strong>
                                <span class="blocker-tip">${escapeHtml(r.status_prosedur || 'Ralan')}</span>
                            </td>
                            <td>
                                <span>${escapeHtml(r.deskripsi_panjang || '-')}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.kode}|${x.status_prosedur}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/procedure/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Procedure (${r.kode || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.deskripsi_panjang || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Procedure ${itemKey.kode}`, 'info');
                const res = await api('/api/modules/procedure/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Procedure terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
