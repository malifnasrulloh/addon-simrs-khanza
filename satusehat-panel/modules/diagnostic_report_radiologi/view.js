/* ============================================================
   Module View: DiagnosticReport Radiologi (Laporan Radiologi)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Laporan Radiologi...</div>`;
    ...
    try {
        const res = await api(`/api/modules/diagnostic_report_radiologi/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada laporan diagnostik radiologi yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span>${escapeHtml((r.hasil_bacaan || r.diagnosa_klinis || '-').slice(0, 100))}</span>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/diagnostic_report_radiologi/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `DiagnosticReport Radiologi (${r.nm_perawatan || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim DiagnosticReport Radiologi ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/diagnostic_report_radiologi/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `DiagnosticReport Radiologi terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
