/* ============================================================
   Module View: Condition (Diagnosa & Keluhan Utama)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Condition...</div>`;
    ...
    try {
        const res = await api(`/api/modules/condition/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada diagnosa yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span class="badge ${r.prioritas === '1' ? 'badge-primary' : 'badge-neutral'}">${escapeHtml(r.category || 'Diagnosa')}</span>
                                <span class="blocker-tip">${escapeHtml(r.status_diagnosa || 'Ralan')}</span>
                            </td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/condition/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Condition (${r.kd_penyakit || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.nm_penyakit || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Condition ${itemKey.kd_penyakit}`, 'info');
                const res = await api('/api/modules/condition/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Condition terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
