...
class Controller extends BaseModuleController
{
    public static function list(): array
    {
        ...
        if ($f['search'] !== '') {
            $where .= " AND (rp.no_rawat LIKE ? OR pj.no_rkm_medis LIKE ? OR pj.nm_pasien LIKE ? OR dp.kd_penyakit LIKE ? OR py.nm_penyakit LIKE ?)";
            $s = "%{$f['search']}%";
        ...
        $sql = "
            SELECT 
                ...
                dp.kd_penyakit, py.nm_penyakit, dp.status as status_diagnosa, dp.prioritas,
            ...
            {$where}
            ORDER BY rp.tgl_registrasi DESC, rp.jam_reg DESC, dp.prioritas ASC
        ...
        try {
            ...
            foreach ($rows as $r) {
                ...
                $localState = self::getLocalState('condition_state', $compositeKey, $r['no_rawat'], 'Condition');
                ...
                $statusInfo = self::evaluateStatus($r, $r['id_condition'], $localState, $blockers);
                ...
                $items[] = array_merge($r, [
                    ...
                    'category'     => ((int)$r['prioritas'] === 1 ? 'Diagnosa Utama (Rank 1)' : 'Diagnosa Sekunder (Rank 2)'),
    ...
    public static function send(): array
    {
        return self::executeSend(
            ...
            function (array|string $itemKey): array {
                ...
                $statusRawat = is_array($itemKey) ? ($itemKey['status'] ?? 'Ralan') : 'Ralan';
                $db = Database::getMysql();
                ...
                throw new \RuntimeException("Payload Condition {$kdPenyakit} tidak ditemukan");
            },
            function (array|string $itemKey, string $satusehatId, array $outcome): void {
                ...
                $stmt = $db->prepare("
                    INSERT INTO satu_sehat_condition (no_rawat, kd_penyakit, status, id_condition)
                    VALUES (?, ?, ?, ?)
