/* ============================================================
   Module View: Specimen Radiologi (Spesimen Radiologi)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Spesimen Radiologi...</div>`;
    ...
    try {
        const res = await api(`/api/modules/specimen_radiologi/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada spesimen radiologi yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong>${escapeHtml(r.sampel_display || r.sampel_code || 'Target Anatomi')}</strong>
                            ...
                            <td><span>${escapeHtml(r.nm_perawatan || '-')}</span></td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/specimen_radiologi/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Specimen Radiologi (${r.sampel_display || r.sampel_code || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Specimen Radiologi ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/specimen_radiologi/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Specimen Radiologi terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
