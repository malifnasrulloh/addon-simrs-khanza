/* ============================================================
   Module View: Encounter (Kunjungan Pasien)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Encounter...</div>`;
    ...
    try {
        const res = await api(`/api/modules/encounter/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada data Encounter yang sesuai filter.</div>`;
        return;
    }
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => x.item_key === key) || {};
            try {
                const prev = await api(`/api/modules/encounter/preview/${encodeURIComponent(key)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Encounter (${key})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.nm_poli || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Encounter ${key} ke SATUSEHAT`, 'info');
                const res = await api('/api/modules/encounter/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [key] })
                });
                if (res.success && res.results?.[key]?.satusehat_id) {
                    toast('Berhasil!', `Encounter terkirim. ID: ${res.results[key].satusehat_id}`, 'success');
                    await loadData();
                } else {
                    const err = res.results?.[key]?.issue_text || res.error || 'Pengiriman gagal';
