/* ============================================================
   Module View: Episode of Care (Program Kesehatan)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Episode of Care...</div>`;
    ...
    try {
        const res = await api(`/api/modules/episode_of_care/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada episode program kesehatan yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.kd_penyakit}|${r.status_diagnosa}`;
                    const canSend = Boolean(st.can_send ?? (st.status !== 'blocked'));
                    const btnLabel = st.status === 'update_needed' ? (st.label || 'Tutup Episode') : 'Kirim';
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span class="badge badge-primary">${escapeHtml(r.eoc_program || '-')}</span>
                            </td>
                            <td>
                                <strong>${escapeHtml(r.kd_penyakit)}</strong>
                                <span class="blocker-tip">${escapeHtml(r.nm_penyakit || '-')}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.kd_penyakit}|${x.status_diagnosa}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/episode_of_care/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Episode of Care (${r.eoc_program || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.kd_penyakit || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Episode of Care ${itemKey.kd_penyakit}`, 'info');
                const res = await api('/api/modules/episode_of_care/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Episode of Care terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
