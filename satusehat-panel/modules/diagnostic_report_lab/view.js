/* ============================================================
   Module View: DiagnosticReport Lab (Hasil & Kesan Lab)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Laporan Diagnostik Lab...</div>`;
    ...
    try {
        const res = await api(`/api/modules/diagnostic_report_lab/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada laporan diagnostik lab yang sesuai filter.</div>`;
        return;
    }
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                ...
                                <span class="blocker-tip mono">Template: ${escapeHtml(String(r.id_template))}</span>
                            </td>
                            <td>
                                <span>${escapeHtml((r.kesan || r.diagnosa_klinis || '-').slice(0, 100))}</span>
                                <span class="blocker-tip">${escapeHtml(r.nm_dokter || '')}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/diagnostic_report_lab/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `DiagnosticReport Lab (${r.Pemeriksaan || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim DiagnosticReport Lab ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/diagnostic_report_lab/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `DiagnosticReport Lab terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
