...
class Controller extends BaseModuleController
{
    public static function list(): array
    {
        ...
        $sql = "
            ...
            LEFT JOIN saran_kesan_lab skl ON skl.no_rawat = per.no_rawat AND skl.tgl_periksa = per.tgl_periksa AND skl.jam = per.jam
            LEFT JOIN pegawai peg ON peg.nik = per.kd_dokter
            LEFT JOIN satu_sehat_encounter sse ON sse.no_rawat = rp.no_rawat
            LEFT JOIN satu_sehat_servicerequest_lab sssl ON sssl.noorder = pdpl.noorder AND sssl.id_template = pdpl.id_template AND sssl.kd_jenis_prw = pdpl.kd_jenis_prw
            ...
            LEFT JOIN satu_sehat_diagnosticreport_lab ssdr ON ssdr.noorder = pdpl.noorder AND ssdr.id_template = pdpl.id_template AND ssdr.kd_jenis_prw = pdpl.kd_jenis_prw
            {$where}
        ...
        try {
            ...
            foreach ($rows as $r) {
                $compositeKey = $r['noorder'] . '_' . $r['id_template'] . '_' . ($r['loinc_code'] ?? '');
                $localState = self::getLocalState('diagnosticreport_lab_pk_state', $compositeKey, $r['no_rawat'], 'DiagnosticReport');
    ...
    public static function send(): array
    {
        return self::executeSend(
            ...
            function (array|string $itemKey): array {
                ...
                throw new \RuntimeException("Payload DiagnosticReport Lab {$noorder} tidak ditemukan");
            },
            function (array|string $itemKey, string $satusehatId, array $outcome): void {
                ...
                $stmt = $db->prepare("
                    INSERT INTO satu_sehat_diagnosticreport_lab (noorder, kd_jenis_prw, id_template, id_diagnosticreport)
                    VALUES (?, ?, ?, ?)
