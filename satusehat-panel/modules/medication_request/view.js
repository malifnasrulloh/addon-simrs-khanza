/* ============================================================
   Module View: MedicationRequest (Resep Obat Dokter)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Resep Obat...</div>`;
    ...
    try {
        const res = await api(`/api/modules/medication_request/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada peresepan obat yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            <thead>
                <tr>
                    ...
                    <th>Pasien & No. Resep</th>
                    <th>Nama Obat</th>
                    <th>Aturan Pakai & Jml</th>
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                ...
                                <span class="blocker-tip mono">Resep: ${escapeHtml(r.no_resep)} · Rawat: ${escapeHtml(r.no_rawat)}</span>
                            </td>
                            <td>
                            ...
                            <td>
                                <span>${escapeHtml(r.aturan_pakai || '-')}</span>
                                <span class="blocker-tip">Jumlah: ${escapeHtml(String(r.jml || 0))}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.no_resep}|${x.kode_brng}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/medication_request/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `MedicationRequest (${r.no_resep || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.nama_brng || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Resep ${itemKey.no_resep}`, 'info');
                const res = await api('/api/modules/medication_request/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `MedicationRequest terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
