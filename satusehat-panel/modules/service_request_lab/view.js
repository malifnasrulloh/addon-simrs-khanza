/* ============================================================
   Module View: ServiceRequest Lab (Permintaan Lab PK & MB)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Permintaan Lab...</div>`;
    ...
    try {
        const res = await api(`/api/modules/service_request_lab/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada permintaan lab yang sesuai filter.</div>`;
        return;
    }
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                ...
                                <span class="blocker-tip mono">Tipe: ${escapeHtml(r.kd_jenis_prw)} · Template: ${escapeHtml(String(r.id_template))}</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/service_request_lab/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `ServiceRequest Lab (${r.Pemeriksaan || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim ServiceRequest Lab ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/service_request_lab/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `ServiceRequest Lab terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
