/* ============================================================
   Module View: ImagingStudy (Studi Citra Medis DICOM / PACS)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Imaging Study DICOM...</div>`;
    ...
    try {
        const res = await api(`/api/modules/imaging_study/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada studi citra radiologi yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.kd_jenis_prw}|${r.tgl_periksa}|${r.jam_periksa}`;
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span class="mono">${escapeHtml(r.acsn || '-')}</span>
                                ${r.noorder ? `<span class="blocker-tip mono muted">Order: ${escapeHtml(r.noorder)}</span>` : ''}
                            </td>
                            <td class="small">${escapeHtml(r.tgl_periksa || '-')} <span class="muted">${escapeHtml(r.jam_periksa || '')}</span></td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.kd_jenis_prw}|${x.tgl_periksa}|${x.jam_periksa}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/imaging_study/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `ImagingStudy (${r.nm_perawatan || ''})`,
                        subtitle: `${r.nm_pasien || ''} · ACSN: ${r.acsn || '-'}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim ImagingStudy ${itemKey.no_rawat}`, 'info');
                const res = await api('/api/modules/imaging_study/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `ImagingStudy terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
