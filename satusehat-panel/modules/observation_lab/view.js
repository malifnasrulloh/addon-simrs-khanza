/* ============================================================
   Module View: Observation Lab (Hasil Pemeriksaan Lab)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Hasil Lab...</div>`;
    ...
    try {
        const res = await api(`/api/modules/observation_lab/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada hasil lab yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.noorder}|${r.kd_jenis_prw}|${r.id_template}`;
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong>${escapeHtml(r.Pemeriksaan || '-')}</strong>
                                <span class="blocker-tip mono">LOINC: ${escapeHtml(r.loinc_code || '-')}</span>
                            </td>
                            <td>
                                <strong class="badge badge-primary font-bold">${escapeHtml(r.nilai || '-')}</strong>
                            </td>
                            <td>
                                <span>${escapeHtml(r.nilai_rujukan || '-')}</span>
                                <span class="blocker-tip">${escapeHtml(r.satuan || '')}</span>
                            </td>
                            <td class="small">${escapeHtml(r.tgl_hasil || '-')} <span class="muted">${escapeHtml(r.jam_hasil || '')}</span></td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.noorder}|${x.kd_jenis_prw}|${x.id_template}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/observation_lab/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Observation Lab (${r.Pemeriksaan || ''})`,
                        subtitle: `Hasil: ${r.nilai || ''} ${r.satuan || ''} · ${r.nm_pasien || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Hasil Lab ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/observation_lab/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Observation Lab terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
