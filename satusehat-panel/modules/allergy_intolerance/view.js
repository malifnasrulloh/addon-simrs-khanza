/* ============================================================
   Module View: AllergyIntolerance (Riwayat Alergi)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Riwayat Alergi...</div>`;
    ...
    try {
        const res = await api(`/api/modules/allergy_intolerance/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada data riwayat alergi yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong style="color:var(--warn-text, inherit)">${escapeHtml(r.alergi)}</strong>
                                <span class="blocker-tip">${escapeHtml(r.status_rawat || 'Ralan')}</span>
                            </td>
                            <td class="small">${escapeHtml(r.tgl_perawatan || '-')} <span class="muted">${escapeHtml(r.jam_rawat || '')}</span></td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/allergy_intolerance/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `AllergyIntolerance (${r.alergi || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Alergi ${r.alergi || ''}`, 'info');
                const res = await api('/api/modules/allergy_intolerance/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `AllergyIntolerance terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
