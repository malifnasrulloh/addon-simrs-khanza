/* ============================================================
   Module View: Specimen Lab (Pengambilan Sampel Lab)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Sampel Lab...</div>`;
    ...
    try {
        const res = await api(`/api/modules/specimen_lab/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada spesimen lab yang sesuai filter.</div>`;
        return;
    }
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong>${escapeHtml(r.sampel_display || r.sampel_code || 'Spesimen')}</strong>
                                <span class="blocker-tip mono">Kode: ${escapeHtml(r.sampel_code || '-')}</span>
                            </td>
                            <td class="small">${escapeHtml(r.tgl_sampel || '-')} <span class="muted">${escapeHtml(r.jam_sampel || '')}</span></td>
                            <td><span>${escapeHtml(r.Pemeriksaan || '-')}</span></td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/specimen_lab/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Specimen Lab (${r.sampel_display || r.sampel_code || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Specimen Lab ${itemKey.noorder}`, 'info');
                const res = await api('/api/modules/specimen_lab/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Specimen Lab terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
