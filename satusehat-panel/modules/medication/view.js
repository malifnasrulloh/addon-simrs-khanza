/* ============================================================
   Module View: Medication (Master Obat KFA)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat master obat KFA...</div>`;
    ...
    try {
        const res = await api(`/api/modules/medication/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada obat yang sesuai filter.</div>`;
        return;
    }
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                ...
                                <span class="blocker-tip mono">Kode: ${escapeHtml(r.kode_brng)}</span>
                            </td>
                            <td>
                                <strong>${escapeHtml(r.obat_code || '-')}</strong>
                                <span class="blocker-tip">${escapeHtml(r.obat_display || '-')}</span>
                            </td>
                            <td>
                                <span>${escapeHtml(r.form_display || r.form_code || '-')}</span>
                                <span class="blocker-tip">${escapeHtml(r.route_display || r.route_code || '-')}</span>
                            </td>
                            <td class="small">
                                ${r.numerator_value ? `${escapeHtml(String(r.numerator_value))} ${escapeHtml(r.numerator_code || '')}` : '-'}
                            </td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => x.kode_brng === keyStr) || {};
            try {
                const prev = await api(`/api/modules/medication/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Medication (${r.nama_brng || ''})`,
                        subtitle: `Kode: ${r.kode_brng} · KFA: ${r.obat_code || '-'}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Master Obat ${keyStr}`, 'info');
                const res = await api('/api/modules/medication/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [keyStr] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Medication terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
