/* ============================================================
   Module View: Observation TTV (Tanda-Tanda Vital)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Tanda Vital (TTV)...</div>`;
    ...
    try {
        const res = await api(`/api/modules/observation_ttv/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada data TTV yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.ttv_type}|${r.tgl_observasi}|${r.jam_observasi}`;
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong>${escapeHtml(r.ttv_label || r.ttv_type)}</strong>
                                <span class="blocker-tip mono muted">LOINC: ${escapeHtml(r.ttv_code || '-')}</span>
                            </td>
                            <td>
                                <span class="badge badge-primary font-bold">${escapeHtml(r.ttv_value)} ${escapeHtml(r.ttv_unit)}</span>
                            </td>
                            <td class="small">${escapeHtml(r.tgl_observasi || '-')} <span class="muted">${escapeHtml(r.jam_observasi || '')}</span></td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.ttv_type}|${x.tgl_observasi}|${x.jam_observasi}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/observation_ttv/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Observation TTV (${r.ttv_label || r.ttv_type})`,
                        subtitle: `${r.nm_pasien || ''} · ${r.ttv_value} ${r.ttv_unit}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim TTV ${itemKey.ttv_type}`, 'info');
                const res = await api('/api/modules/observation_ttv/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Observation TTV terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
