/* ============================================================
   Module View: MedicationDispense (Penyerahan / Dispensing Obat)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Dispense Obat...</div>`;
    ...
    try {
        const res = await api(`/api/modules/medication_dispense/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada penyerahan obat yang sesuai filter.</div>`;
        return;
    }
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                ...
                                <span class="blocker-tip mono">Batch: ${escapeHtml(r.no_batch || '-')} · Faktur: ${escapeHtml(r.no_faktur || '-')}</span>
                            </td>
                            <td>
                            ...
                            <td><span class="badge badge-neutral">${escapeHtml(r.status_pemberian || 'Ralan')}</span></td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const prev = await api(`/api/modules/medication_dispense/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `MedicationDispense (${r.nama_brng || ''})`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Dispense ${itemKey.kode_brng}`, 'info');
                const res = await api('/api/modules/medication_dispense/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `MedicationDispense terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
