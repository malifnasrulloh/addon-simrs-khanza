...
class Controller extends BaseModuleController
{
    public static function list(): array
    {
        ...
        if ($f['search'] !== '') {
            $where .= " AND (rp.no_rawat LIKE ? OR pj.no_rkm_medis LIKE ? OR pj.nm_pasien LIKE ? OR pem.penilaian LIKE ?)";
            $s = "%{$f['search']}%";
        ...
        $sql = "
            SELECT 
                ...
                pem.penilaian, pem.keluhan, pem.pemeriksaan, pem.tgl_perawatan, pem.jam_rawat, 'Ralan' as status_rawat,
                ssc.kd_penyakit, py.nm_penyakit,
            ...
            LEFT JOIN satu_sehat_clinicalimpression ssci ON ssci.no_rawat = pem.no_rawat AND ssci.tgl_perawatan = pem.tgl_perawatan AND ssci.jam_rawat = pem.jam_rawat AND ssci.status = 'Ralan'
            {$where} AND pem.penilaian IS NOT NULL AND pem.penilaian <> ''
            ORDER BY pem.tgl_perawatan DESC, pem.jam_rawat DESC
        ...
        try {
            ...
            foreach ($rows as $r) {
                $compositeKey = $r['no_rawat'] . '_' . $r['tgl_perawatan'] . '_' . $r['jam_rawat'] . '_' . $r['status_rawat'] . ($r['kd_penyakit'] ? '_' . $r['kd_penyakit'] : '');
                $localState = self::getLocalState('clinical_impression_state', $compositeKey, $r['no_rawat'], 'ClinicalImpression');
                ...
                $statusInfo = self::evaluateStatus($r, $r['id_clinicalimpression'], $localState, $blockers);
