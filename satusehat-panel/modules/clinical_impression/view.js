/* ============================================================
   Module View: ClinicalImpression (Penilaian Klinis Dokter)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Clinical Impression...</div>`;
    ...
    try {
        const res = await api(`/api/modules/clinical_impression/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada penilaian klinis yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            <thead>
                <tr>
                    ...
                    <th>Pasien</th>
                    <th>Penilaian / Evaluasi Klinis</th>
                    <th>Diagnosa Terkait</th>
                ...
                ${list.map(r => {
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <span>${escapeHtml((r.penilaian || '-').slice(0, 120))}</span>
                                <span class="blocker-tip">${escapeHtml(r.status_rawat || 'Ralan')} · ${escapeHtml(r.tgl_perawatan || '')} ${escapeHtml(r.jam_rawat || '')}</span>
                            </td>
                            <td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.tgl_perawatan}|${x.jam_rawat}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/clinical_impression/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `ClinicalImpression`,
                        subtitle: `${r.nm_pasien || ''} · ${r.tgl_perawatan || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim ClinicalImpression`, 'info');
                const res = await api('/api/modules/clinical_impression/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `ClinicalImpression terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
