/* ============================================================
   Module View: Immunization (Vaksinasi & Imunisasi)
   ============================================================ */
...
async function loadData() {
    ...
    currentContainer.innerHTML = `<div class="empty-state"><span class="spinner"></span> Memuat data Vaksinasi...</div>`;
    ...
    try {
        const res = await api(`/api/modules/immunization/list?${qs.toString()}`);
        items = res.data || [];
        renderTable(items);
    } catch (e) {
...
function renderTable(list) {
    if (!list.length) {
        currentContainer.innerHTML = `<div class="empty-state">Tidak ada data vaksinasi yang sesuai filter.</div>`;
        return;
    }
        ...
        <table class="patient-table">
            ...
            <tbody>
                ${list.map(r => {
                    ...
                    const keyStr = `${r.no_rawat}|${r.kode_brng}|${r.no_batch}`;
                    ...
                    return `
                        <tr data-key="${escapeHtml(keyStr)}">
                            ...
                            <td>
                                <strong>${escapeHtml(r.vaksin_display || r.kode_brng)}</strong>
                                <span class="blocker-tip mono">Kode: ${escapeHtml(r.vaksin_code || '-')}</span>
                            </td>
                            <td>
                                <span>Batch: ${escapeHtml(r.no_batch || '-')}</span>
                                <span class="blocker-tip">${escapeHtml(r.aturan || 'Dosis 1')} · ${escapeHtml(String(r.jml || 1))} vial</span>
                            </td>
    ...
    currentContainer.querySelectorAll('.btn-inspect').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const r = list.find(x => `${x.no_rawat}|${x.kode_brng}|${x.no_batch}` === keyStr) || {};
            try {
                const prev = await api(`/api/modules/immunization/preview/${encodeURIComponent(keyStr)}`);
                if (currentOptions?.onInspect) {
                    currentOptions.onInspect({
                        title: `Immunization (${r.vaksin_display || ''})`,
                        subtitle: `${r.nm_pasien || ''} · Batch: ${r.no_batch || ''}`,
    ...
    currentContainer.querySelectorAll('.btn-send-single').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                toast('Mengirim...', `Mengirim Vaksin ${itemKey.kode_brng}`, 'info');
                const res = await api('/api/modules/immunization/send', {
                    method: 'POST',
                    body: JSON.stringify({ items: [itemKey] })
                });
                if (res.success && res.results?.[keyStr]?.satusehat_id) {
                    toast('Berhasil!', `Immunization terkirim. ID: ${res.results[keyStr].satusehat_id}`, 'success');
                    await loadData();
                } else {
