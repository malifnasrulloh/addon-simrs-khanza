...
<?php
/**
 * SIMRS Khanza - Satu Sehat Diagnostic Report (Lab MB) Sync Service
 *
 * CLI service to synchronize patient Diagnostic Report (Lab MB) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_diagnosticreportlabmb_sync.php                  # Normal run
 *   php satusehat_diagnosticreportlabmb_sync.php --verbose        # Extra debug output
 *   php satusehat_diagnosticreportlabmb_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_diagnosticreportlabmb_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_diagnosticreportlabmb_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatDiagnosticReportLabMbSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat Diagnostic Report (Lab MB) Sync   ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_diagnosticreportlabmb_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_diagnosticreportlabmb_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_diagnostic_report_lab_mb', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Diagnostic Report (Lab MB) Sync");
...
require_once BASE_DIR . '/lib/satusehat/DiagnosticReportLabMbProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('diagnosticreport_lab_mb', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatDiagnosticReportLabMbProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New DiagnosticReportLabMB (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportLabMBActive', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportLabMB/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update DiagnosticReportLabMB (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportLabMBUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportLabMB/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
