...
$activeRecords = $db->fetchPendingDiagnosticReportLabMBActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingDiagnosticReportLabMBUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Diagnostic Report (Lab MB) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update Diagnostic Reports pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatDiagnosticReportLabMbProcessor($childDb, $childClient, $config, $log);
