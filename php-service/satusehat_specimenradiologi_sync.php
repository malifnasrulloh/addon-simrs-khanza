...
<?php
/**
 * SIMRS Khanza - Satu Sehat Specimen (Radiologi) Sync Service
 *
 * CLI service to synchronize patient Specimen (Radiologi) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_specimenradiologi_sync.php                  # Normal run
 *   php satusehat_specimenradiologi_sync.php --verbose        # Extra debug output
 *   php satusehat_specimenradiologi_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_specimenradiologi_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_specimenradiologi_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatSpecimenRadiologiSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat Specimen (Radiologi) Sync         ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_specimenradiologi_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_specimenradiologi_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_specimen_radiologi', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Specimen (Radiologi) Sync");
...
require_once BASE_DIR . '/lib/satusehat/SpecimenRadiologiProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('specimen_radiologi', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatSpecimenRadiologiProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New SpecimenRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingSpecimenRadiologiActive', [$dateFrom, $dateTo], $batchSize, $log, 'SpecimenRadiologi/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update SpecimenRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingSpecimenRadiologiUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'SpecimenRadiologi/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
