...
$activeRecords = $db->fetchPendingSpecimenRadiologiActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingSpecimenRadiologiUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Specimen (Radiologi) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update Specimens pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatSpecimenRadiologiProcessor($childDb, $childClient, $config, $log);
