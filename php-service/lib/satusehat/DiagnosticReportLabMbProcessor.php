...
/**
 * DiagnosticReportLabMbProcessor - Orchestrator for Satu Sehat Diagnostic Report (Lab MB) sync.
 *
...
class SatuSehatDiagnosticReportLabMbProcessor
{
    ...
    public function run(?array $activeRecords = null, ?array $updateRecords = null): array
    {
        ...
        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 1: POST New Diagnostic Report (Lab MB)");
        $this->processActive($dateFrom, $dateTo, $activeRecords);

        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 2: PUT Update Diagnostic Report (Lab MB)");
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingDiagnosticReportLabMBActive($dateFrom, $dateTo);
        }
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingDiagnosticReportLabMBUpdate($dateFrom, $dateTo);
        }

        if (empty($records)) {
            $this->log->info("[PHASE 2] No pending Diagnostic Reports to PUT.");
            return;
        }
        ...
        foreach ($records as $p) {
            ...
            $this->log->info("[PHASE 2] {$noorder} [{$idTemplate}/{$kdJenisPrw}]: PUT /DiagnosticReport/{$idDiagnosticReport} ({$pemeriksaan})");
