...
/**
 * ObservationLabMbProcessor - Orchestrator for Satu Sehat Observation (Lab MB) sync.
 *
...
class SatuSehatObservationLabMbProcessor
{
    ...
    public function run(?array $activeRecords = null, ?array $updateRecords = null): array
    {
        ...
        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 1: POST New Observation (Lab MB)");
        $this->processActive($dateFrom, $dateTo, $activeRecords);

        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 2: PUT Update Observation (Lab MB)");
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingObservationLabMBActive($dateFrom, $dateTo);
        }
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingObservationLabMBUpdate($dateFrom, $dateTo);
        }
