...
/**
 * SpecimenLabMbProcessor - Orchestrator for Satu Sehat Specimen (Lab MB) sync.
 *
...
class SatuSehatSpecimenLabMbProcessor
{
    ...
    public function run(?array $activeRecords = null, ?array $updateRecords = null): array
    {
        ...
        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 1: POST New Specimen (Lab MB)");
        $this->processActive($dateFrom, $dateTo, $activeRecords);

        $this->log->info("──────────────────────────────────────────────────────────────");
        $this->log->info("[SYNC] Phase 2: PUT Update Specimen (Lab MB)");
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingSpecimenLabMBActive($dateFrom, $dateTo);
        }
        ...
        foreach ($records as $p) {
            ...
            $localState = $this->db->getSpecimenLabMBLocalState($noorder, $kdJenisPrw, $idTemplate);
            ...
            if (empty($sampelCode)) {
                ...
                $this->db->updateSpecimenLabMBLocalState($noorder, $kdJenisPrw, $idTemplate, 'invalid_code');
                $this->skipCount++;
                continue;
            }
            ...
            if ($result['success'] && isset($result['data']['id'])) {
                ...
                $this->db->saveSpecimenLabMB($noorder, $kdJenisPrw, $idTemplate, $idSpecimen);
                $this->db->updateSpecimenLabMBLocalState($noorder, $kdJenisPrw, $idTemplate, 'active');
                ...
                if ($isDuplicate) {
                    ...
                    if (!$isTransient) {
                        ...
                        if ($state !== 'fail') {
                            $this->db->updateSpecimenLabMBLocalState($noorder, $kdJenisPrw, $idTemplate, $state);
                        }
                    }
                    $this->failCount++;
                }
    ...
    {
        if ($records === null) {
            $records = $this->db->fetchPendingSpecimenLabMBUpdate($dateFrom, $dateTo);
        }
