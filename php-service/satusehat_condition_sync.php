...
<?php
/**
 * SIMRS Khanza - Satu Sehat Condition Sync Service
 *
 * CLI service to synchronize patient diagnosis/condition data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_condition_sync.php                  # Normal run
 *   php satusehat_condition_sync.php --verbose        # Extra debug output
 *   php satusehat_condition_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_condition_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_condition_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatConditionSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_condition_sync.php [options]
    ...
    Cron (every 5 minutes):
      */5 * * * * cd /path/to/php-service && php satusehat_condition_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_condition', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Condition Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/ConditionProcessor.php';
...
// Resolve dates matching ConditionProcessor
...
$processor = new SatuSehatConditionProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New Condition (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingConditionActive', [$dateFrom, $dateTo], $batchSize, $log, 'Condition/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update Condition (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingConditionUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'Condition/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
