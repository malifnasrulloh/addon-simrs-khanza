...
$activeRecords = $db->fetchPendingServiceRequestLabMBActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingServiceRequestLabMBUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending ServiceRequest (Lab MB) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update ServiceRequests pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatServiceRequestLabMbProcessor($childDb, $childClient, $config, $log);
