...
$activeRecords = $db->fetchPendingObservationLabPKActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingObservationLabPKUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Observation (Lab PK) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatObservationLabPkProcessor($childDb, $childClient, $config, $log);
