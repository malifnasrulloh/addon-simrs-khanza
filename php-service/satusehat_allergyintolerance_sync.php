...
<?php
/**
 * SIMRS Khanza - Satu Sehat AllergyIntolerance Sync Service
 *
 * CLI service to synchronize patient AllergyIntolerance data with Satu Sehat API.
 ...
 *   php satusehat_allergyintolerance_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_allergyintolerance_sync.php --parallel=8     # Run with 8 concurrent workers
...
define('SERVICE_NAME', 'KhanzaSatuSehatAllergySync');
...
if (isset($options['help'])) {
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_allergyintolerance_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_allergyintolerance', $logLevel, $isVerbose);
...
// Resolve dates matching AllergyIntoleranceProcessor
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New Allergy (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingAllergyActive', [$dateFrom, $dateTo], $batchSize, $log, 'Allergy/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update Allergy (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingAllergyUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'Allergy/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
