...
$activeRecords = $db->fetchPendingDiagnosticReportLabPKActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingDiagnosticReportLabPKUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Diagnostic Report (Lab PK) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatDiagnosticReportLabPkProcessor($childDb, $childClient, $config, $log);
