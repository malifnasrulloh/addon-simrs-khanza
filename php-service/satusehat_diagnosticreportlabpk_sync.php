...
<?php
/**
 * SIMRS Khanza - Satu Sehat Diagnostic Report (Lab PK) Sync Service
 *
 * CLI service to synchronize patient Diagnostic Report (Lab PK) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_diagnosticreportlabpk_sync.php                  # Normal run
 *   php satusehat_diagnosticreportlabpk_sync.php --verbose        # Extra debug output
 *   php satusehat_diagnosticreportlabpk_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_diagnosticreportlabpk_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_diagnosticreportlabpk_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatDiagnosticReportLabPkSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat Diagnostic Report (Lab PK) Sync   ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_diagnosticreportlabpk_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_diagnosticreportlabpk_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_diagnostic_report_lab_pk', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Diagnostic Report (Lab PK) Sync");
...
require_once BASE_DIR . '/lib/satusehat/DiagnosticReportLabPkProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('diagnosticreport_lab_pk', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatDiagnosticReportLabPkProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New DiagnosticReportLabPK (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportLabPKActive', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportLabPK/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update DiagnosticReportLabPK (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportLabPKUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportLabPK/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
