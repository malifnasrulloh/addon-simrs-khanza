...
$activeRecords = $db->fetchPendingCompositionActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingCompositionUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Composition records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update compositions pending sync.");

// Execution path: Parallel workers vs Single process
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatCompositionProcessor($childDb, $childClient, $config, $log);
