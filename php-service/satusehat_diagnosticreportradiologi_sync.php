...
<?php
/**
 * SIMRS Khanza - Satu Sehat DiagnosticReport (Radiologi) Sync Service
 *
 * CLI service to synchronize patient DiagnosticReport (Radiologi) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_diagnosticreportradiologi_sync.php                  # Normal run
 *   php satusehat_diagnosticreportradiologi_sync.php --verbose        # Extra debug output
 *   php satusehat_diagnosticreportradiologi_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_diagnosticreportradiologi_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_diagnosticreportradiologi_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatDiagnosticReportRadiologiSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat DiagnosticReport (Radiologi) Sync ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_diagnosticreportradiologi_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_diagnosticreportradiologi_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_diagnosticreport_radiologi', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat DiagnosticReport (Radiologi) Sync");
...
$processor = new SatuSehatDiagnosticReportRadiologiProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New DiagnosticReportRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportRadiologiActive', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportRadiologi/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update DiagnosticReportRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingDiagnosticReportRadiologiUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'DiagnosticReportRadiologi/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
