...
<?php
/**
 * SIMRS Khanza - Satu Sehat MedicationRequest Sync Service
 *
 * CLI service to synchronize patient MedicationRequest data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_medicationrequest_sync.php                  # Normal run
 *   php satusehat_medicationrequest_sync.php --verbose        # Extra debug output
 *   php satusehat_medicationrequest_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_medicationrequest_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_medicationrequest_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatMedicationRequestSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_medicationrequest_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_medicationrequest_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_medicationrequest', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat MedicationRequest Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/MedicationRequestProcessor.php';
...
// Resolve dates matching MedicationRequestProcessor
...
$processor = new SatuSehatMedicationRequestProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New MedicationRequest (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingMedicationRequestActive', [$dateFrom, $dateTo], $batchSize, $log, 'MedicationRequest/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update MedicationRequest (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingMedicationRequestUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'MedicationRequest/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
