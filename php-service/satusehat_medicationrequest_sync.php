...
if ($totalPending === 0) {
    $log->info("No pending MedicationRequest records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update medication requests pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatMedicationRequestProcessor($childDb, $childClient, $config, $log);
