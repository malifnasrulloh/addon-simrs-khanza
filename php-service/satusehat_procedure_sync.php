...
<?php
/**
 * SIMRS Khanza - Satu Sehat Procedure Sync Service
 *
 * CLI service to synchronize patient Procedure data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_procedure_sync.php                  # Normal run
 *   php satusehat_procedure_sync.php --verbose        # Extra debug output
 *   php satusehat_procedure_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_procedure_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_procedure_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatProcedureSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_procedure_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_procedure_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_procedure', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Procedure Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/ProcedureProcessor.php';
...
// Resolve dates matching ProcedureProcessor
...
$processor = new SatuSehatProcedureProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New Procedure (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingProcedureActive', [$dateFrom, $dateTo], $batchSize, $log, 'Procedure/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update Procedure (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingProcedureUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'Procedure/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
