...
<?php
/**
 * SIMRS Khanza - Satu Sehat ImagingStudy Sync Service
 *
 * CLI service to synchronize patient Radiology images/studies with Satu Sehat / PACS / Kemenkes DICOM Router.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_imagingstudy_sync.php                  # Normal run
 *   php satusehat_imagingstudy_sync.php --verbose        # Extra debug output
 *   php satusehat_imagingstudy_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_imagingstudy_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_imagingstudy_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatImagingStudySync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat ImagingStudy Sync                 ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_imagingstudy_sync.php [options]
    ...
    Environment:
      Configure SATUSEHAT_* and ORTHANC_* / DICOM_CONVERTER_* variables in .env.

    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_imagingstudy_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_imagingstudy', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat ImagingStudy Sync");
...
$log->info("  Orthanc URL: " . $config->orthancUrl . ":" . $config->orthancPort);
$log->info("══════════════════════════════════════════════════════════════");
...
require_once BASE_DIR . '/lib/satusehat/ImagingStudyProcessor.php';

$orthanc = new OrthancClient($config, $log);

try {
    // SatuSehatDatabase constructor needs $client which is used for logging/updating. We can pass null or dummy if we don't call SATUSEHAT FHIR API directly,
    // but Database.php expects the SatuSehatClient object or null. Let's see if Database.php accepts null.
    // Yes, the third param is optional: function __construct(SatuSehatConfig $config, Logger $log, ?SatuSehatClient $api = null)
    $db = new SatuSehatDatabase($config, $log, null);
...
// Run Diagnostics
$db->printSyncDiagnostics('imagingstudy_radiologi', $dateFrom, $dateTo);
