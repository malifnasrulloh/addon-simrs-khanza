...
<?php
/**
 * SIMRS Khanza - Satu Sehat MedicationDispense Sync Service
 *
 * CLI service to synchronize patient MedicationDispense data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_medicationdispense_sync.php                  # Normal run
 *   php satusehat_medicationdispense_sync.php --verbose        # Extra debug output
 *   php satusehat_medicationdispense_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_medicationdispense_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_medicationdispense_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatMedicationDispenseSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_medicationdispense_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_medicationdispense_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_medicationdispense', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat MedicationDispense Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/MedicationDispenseProcessor.php';
...
if ($totalPending === 0) {
    $log->info("No pending MedicationDispense records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update medication dispenses pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatMedicationDispenseProcessor($childDb, $childClient, $config, $log);
    ...
    $processor = new SatuSehatMedicationDispenseProcessor($db, $client, $config, $log);
    try {
