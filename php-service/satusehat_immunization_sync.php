...
$activeRecords = $db->fetchPendingImmunizationActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingImmunizationUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Immunization records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update immunizations pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatImmunizationProcessor($childDb, $childClient, $config, $log);
