...
$log = new Logger($config->logDir, 'satusehat_episodeofcare', $logLevel, $isVerbose);
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New Eoc (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingEocActive', [$dateFrom, $dateTo], $batchSize, $log, 'Eoc/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update Eoc (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingEocFinished', [$dateFrom, $dateTo], $batchSize, $log, 'Eoc/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
