...
$activeRecords = $db->fetchPendingQuestionnaireResponseActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingQuestionnaireResponseUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending QuestionnaireResponse records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update QuestionnaireResponse records pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatQuestionnaireResponseProcessor($childDb, $childClient, $config, $log);
