...
<?php
/**
 * SIMRS Khanza - Satu Sehat QuestionnaireResponse Sync Service
 *
 * CLI service to synchronize patient prescription review (telaah_farmasi) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_questionnaireresponse_sync.php                  # Normal run
 *   php satusehat_questionnaireresponse_sync.php --verbose        # Extra debug output
 *   php satusehat_questionnaireresponse_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_questionnaireresponse_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_questionnaireresponse_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatQuestionnaireResponseSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_questionnaireresponse_sync.php [options]
    ...
    Cron (every 5 minutes):
      */5 * * * * cd /path/to/php-service && php satusehat_questionnaireresponse_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_questionnaireresponse', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat QuestionnaireResponse Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/QuestionnaireResponseProcessor.php';
...
// Resolve dates matching Processor
...
// Run Diagnostics
$db->printSyncDiagnostics('questionnaireresponse', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatQuestionnaireResponseProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New QuestionnaireResponse (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingQuestionnaireResponseActive', [$dateFrom, $dateTo], $batchSize, $log, 'QuestionnaireResponse/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update QuestionnaireResponse (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingQuestionnaireResponseUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'QuestionnaireResponse/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
