...
$activeRecords = $db->fetchPendingSpecimenLabMBActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingSpecimenLabMBUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Specimen (Lab MB) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatSpecimenLabMbProcessor($childDb, $childClient, $config, $log);
