...
$activeRecords = $db->fetchPendingServiceRequestRadiologiActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingServiceRequestRadiologiUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending ServiceRequest (Radiologi) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatServiceRequestRadiologiProcessor($childDb, $childClient, $config, $log);
