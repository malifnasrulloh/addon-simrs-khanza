...
<?php
/**
 * SIMRS Khanza - Satu Sehat Observation (Lab MB) Sync Service
 *
 * CLI service to synchronize patient Observation (Lab MB) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_observationlabmb_sync.php                  # Normal run
 *   php satusehat_observationlabmb_sync.php --verbose        # Extra debug output
 *   php satusehat_observationlabmb_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_observationlabmb_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_observationlabmb_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatObservationLabMbSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat Observation (Lab MB) Sync         ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_observationlabmb_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_observationlabmb_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_observation_lab_mb', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Observation (Lab MB) Sync");
...
require_once BASE_DIR . '/lib/satusehat/ObservationLabMbProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('observation_lab_mb', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatObservationLabMbProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New ObservationLabMB (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingObservationLabMBActive', [$dateFrom, $dateTo], $batchSize, $log, 'ObservationLabMB/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update ObservationLabMB (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingObservationLabMBUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'ObservationLabMB/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
