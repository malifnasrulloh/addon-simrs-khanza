...
$activeRecords = $db->fetchPendingObservationLabMBActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingObservationLabMBUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Observation (Lab MB) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatObservationLabMbProcessor($childDb, $childClient, $config, $log);
