...
<?php
/**
 * SIMRS Khanza - Satu Sehat NutritionOrder Sync Service
 *
 * CLI service to synchronize patient NutritionOrder (Intervensi Gizi Oral) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 *
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_nutritionorder_sync.php                  # Normal run
 *   php satusehat_nutritionorder_sync.php --verbose        # Extra debug output
 *   php satusehat_nutritionorder_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_nutritionorder_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_nutritionorder_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatNutritionOrderSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_nutritionorder_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_nutritionorder_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_nutritionorder', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat NutritionOrder Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/NutritionOrderProcessor.php';
...
// Resolve dates matching NutritionOrderProcessor
...
$processor = new SatuSehatNutritionOrderProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New NutritionOrder (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingNutritionOrderActive', [$dateFrom, $dateTo], $batchSize, $log, 'NutritionOrder/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT/PATCH Update NutritionOrder (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingNutritionOrderUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'NutritionOrder/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
