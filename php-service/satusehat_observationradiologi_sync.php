...
$activeRecords = $db->fetchPendingObservationRadiologiActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingObservationRadiologiUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Observation (Radiologi) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update Observations pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatObservationRadiologiProcessor($childDb, $childClient, $config, $log);
