...
<?php
/**
 * SIMRS Khanza - Satu Sehat Observation (Radiologi) Sync Service
 *
 * CLI service to synchronize patient Observation (Radiologi) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_observationradiologi_sync.php                  # Normal run
 *   php satusehat_observationradiologi_sync.php --verbose        # Extra debug output
 *   php satusehat_observationradiologi_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_observationradiologi_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_observationradiologi_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatObservationRadiologiSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat Observation (Radiologi) Sync      ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_observationradiologi_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_observationradiologi_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_observation_radiologi', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat Observation (Radiologi) Sync");
...
require_once BASE_DIR . '/lib/satusehat/ObservationRadiologiProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('observation_radiologi', $dateFrom, $dateTo);
...
$processor = new SatuSehatObservationRadiologiProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New ObservationRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingObservationRadiologiActive', [$dateFrom, $dateTo], $batchSize, $log, 'ObservationRadiologi/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update ObservationRadiologi (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingObservationRadiologiUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'ObservationRadiologi/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
