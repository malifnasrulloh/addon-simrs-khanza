...
<?php
/**
 * SIMRS Khanza - Satu Sehat CarePlan Sync Service
 *
 * CLI service to synchronize patient CarePlan data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_careplan_sync.php                  # Normal run
 *   php satusehat_careplan_sync.php --verbose        # Extra debug output
 *   php satusehat_careplan_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_careplan_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_careplan_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatCarePlanSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_careplan_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_careplan_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_careplan', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat CarePlan Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/CarePlanProcessor.php';
...
// Resolve dates matching CarePlanProcessor
...
// Run Diagnostics
$db->printSyncDiagnostics('careplan', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatCarePlanProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New CarePlan (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingCarePlanActive', [$dateFrom, $dateTo], $batchSize, $log, 'CarePlan/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update CarePlan (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingCarePlanUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'CarePlan/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
