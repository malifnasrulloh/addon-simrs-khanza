...
$activeRecords = $db->fetchPendingCarePlanActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingCarePlanUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending CarePlan records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update care plans pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatCarePlanProcessor($childDb, $childClient, $config, $log);
