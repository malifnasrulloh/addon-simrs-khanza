...
$activeRecords = $db->fetchPendingClinicalImpressionActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingClinicalImpressionUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending ClinicalImpression records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}

$log->info("[INIT] Found {$totalActive} active and {$totalUpdate} update clinical impressions pending sync.");
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatClinicalImpressionProcessor($childDb, $childClient, $config, $log);
