...
<?php
/**
 * SIMRS Khanza - Satu Sehat ClinicalImpression Sync Service
 *
 * CLI service to synchronize patient Clinical Impression data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_clinicalimpression_sync.php                  # Normal run
 *   php satusehat_clinicalimpression_sync.php --verbose        # Extra debug output
 *   php satusehat_clinicalimpression_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_clinicalimpression_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_clinicalimpression_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatClinicalImpressionSync');
...
if (isset($options['help'])) {
    ...
    Usage:
      php satusehat_clinicalimpression_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_clinicalimpression_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_clinicalimpression', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat ClinicalImpression Sync Service");
...
require_once BASE_DIR . '/lib/satusehat/ClinicalImpressionProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('clinical_impression', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatClinicalImpressionProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New ClinicalImpression (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingClinicalImpressionActive', [$dateFrom, $dateTo], $batchSize, $log, 'ClinicalImpression/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update ClinicalImpression (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingClinicalImpressionUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'ClinicalImpression/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
