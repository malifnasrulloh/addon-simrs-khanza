...
$activeRecords = $db->fetchPendingSpecimenLabPKActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingSpecimenLabPKUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending Specimen (Lab PK) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatSpecimenLabPkProcessor($childDb, $childClient, $config, $log);
