...
$activeRecords = $db->fetchPendingServiceRequestLabPKActive($dateFrom, $dateTo);
$updateRecords = $db->fetchPendingServiceRequestLabPKUpdate($dateFrom, $dateTo);
...
if ($totalPending === 0) {
    $log->info("No pending ServiceRequest (Lab PK) records found.");
    $log->info("══════════════════════════════════════════════════════════════");
    $db->close();
    exit(0);
}
...
if ($isParallel && $totalPending > 1) {
    ...
    for ($i = 0; $i < $numWorkers; $i++) {
        ...
        if ($pid === -1) {
            ...
            $processor = new SatuSehatServiceRequestLabPkProcessor($childDb, $childClient, $config, $log);
