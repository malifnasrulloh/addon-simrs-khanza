...
<?php
/**
 * SIMRS Khanza - Satu Sehat ServiceRequest (Lab PK) Sync Service
 *
 * CLI service to synchronize patient ServiceRequest (Lab PK) data with Satu Sehat API.
 * Supports multi-process parallel execution (--parallel) using pcntl_fork for blazing fast speeds.
 * 
 * Designed to run as a cron job.
 *
 * Usage:
 *   php satusehat_servicerequestlabpk_sync.php                  # Normal run
 *   php satusehat_servicerequestlabpk_sync.php --verbose        # Extra debug output
 *   php satusehat_servicerequestlabpk_sync.php --parallel       # Run with 4 concurrent workers
 *   php satusehat_servicerequestlabpk_sync.php --parallel=8     # Run with 8 concurrent workers
 *   php satusehat_servicerequestlabpk_sync.php --help           # Show help
 *
...
define('SERVICE_NAME', 'KhanzaSatuSehatServiceRequestLabPkSync');
...
if (isset($options['help'])) {
    ...
    echo <<<HELP
    ╔══════════════════════════════════════════════════════════════╗
    ║  SIMRS Khanza - Satu Sehat ServiceRequest (Lab PK) Sync      ║
    ║  Version {$ver}                                              ║
    ╚══════════════════════════════════════════════════════════════╝

    Usage:
      php satusehat_servicerequestlabpk_sync.php [options]
    ...
    Cron (every 10 minutes):
      */10 * * * * cd /path/to/php-service && php satusehat_servicerequestlabpk_sync.php --parallel
...
$log = new Logger($config->logDir, 'satusehat_servicerequest_lab_pk', $logLevel, $isVerbose);
$log->cleanOldLogs($config->logRetentionDays);

$log->info("══════════════════════════════════════════════════════════════");
$log->info("  SIMRS Khanza - Satu Sehat ServiceRequest (Lab PK) Sync");
...
require_once BASE_DIR . '/lib/satusehat/ServiceRequestLabPkProcessor.php';
...
// Run Diagnostics
$db->printSyncDiagnostics('servicerequest_lab_pk', $dateFrom, $dateTo);

// 1. Fetch pending records
$batchSize = $config->batchSize;
$processor = new SatuSehatServiceRequestLabPkProcessor($db, $client, $config, $log);
$totalSuccess = 0;
$totalFail = 0;
$totalSkip = 0;
...
try {
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 1: POST New ServiceRequestLabPK (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingServiceRequestLabPKActive', [$dateFrom, $dateTo], $batchSize, $log, 'ServiceRequestLabPK/active');
    ...
    $log->info("──────────────────────────────────────────────────────────────");
    $log->info("[SYNC] Phase 2: PUT Update ServiceRequestLabPK (batches of {$batchSize})");
    $cursor = new SatuSehatBatchCursor($db, 'fetchPendingServiceRequestLabPKUpdate', [$dateFrom, $dateTo], $batchSize, $log, 'ServiceRequestLabPK/update');
    foreach ($cursor->batches() as $batch) {
        $stats = $processor->run([], $batch);
        $totalSuccess += $stats['success'];
        $totalFail += $stats['fail'];
